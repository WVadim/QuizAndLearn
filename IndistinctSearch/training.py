...
while True:
    ...
    data = build_dataset(sample_high=(index + 1) * bsize, sample_low=index * bsize)
    index += 1
    if len(data) == 0:
        break
        ...
        for train_index, test_index in split.split(X_all):
            ...
            for i in range(0, dataset_size - batch_size, batch_size):
                ...
                if lcounter % 10 == 0:
                    ...
                    print('order %d, shuffle %d, step %d out of %d, training accuracy %g, training loss %g' %
                          (order, shuffle_counter, i, dataset_size, train_accuracy, loss_val))
                lcounter += 1
    if save:
...
while True:
    ...
    data_evaluate = build_dataset(sample_high=(index + 1) * bsize, sample_low=index * bsize, path='test.csv', no_answer=True)
